...
path = f"/Volumes/NINET/CARTBIND_EEG_data/0_CARTBIND_UBC"
save_path = f"/Volumes/NINET/CARTBIND_EEG_data/CARTBIND_UBC"
...
def autoreject_bads_ch(signal, report, zscore_thresh, n_fft=2048, fmax=45.0):
    ...
    # print(f'Lower bound: {bad_lower_bound}\nUpper bound: {bad_upper_bound}')
    # print("Bad ch: ", bad_ch)
    # print("Bad bound ch: ", bad_bound_ch)
...
def main():
    ...
    folders = glob(f"{path}/*")
    epoch_num = []
    for folder in sorted(folders)[21:]:
        ...
        for subfolder in subfolders:
            ...
            crop_signal(raw=raw)
            raw.load_data()
            set_montage(raw=raw, report=report)
...
if __name__ =="__main__":
    main()
    #Test script
    file_path = '/Volumes/NINET/CARTBIND_EEG_data/0_CARTBIND_UBC/CBN02_0016/T0/CBN02_0016_REST_EC_T0_UBC.vhdr'
    raw = mne.io.read_raw_brainvision(vhdr_fname=file_path, preload=True)
    raw.filter(l_freq=0.5, h_freq=45.0, h_trans_bandwidth=5.0)
    spectrum = raw.compute_psd(fmax=45, picks='eeg', n_fft=2048*3)
    psd, freq = spectrum.get_data(return_freqs=True)
    ch_names = spectrum.ch_names
    print("psd size: ", psd.shape)
    print("freq size: ", freq.shape)
    ...
    freq = freq[24:]
    mean_ch_power = np.mean(psd_scaled, axis=0) #scalar val per channel (only eeg)
    median_ch_power = np.median(psd_scaled, axis=0)
    #calculate mad
    # mad_ch_power = np.median(np.abs(psd_scaled - mean_ch_power), axis=0)

    #Calculate the variance for the flatness
    var_ch_power = np.var(psd_scaled, axis=1)
    std_power = np.std(psd_scaled, axis=0)
    print(f"mean psd shape: {mean_ch_power.shape}\nmedian psd shape: {median_ch_power.shape}")
    print(f"max abs difference between mean and median: {np.max(abs(mean_ch_power - median_ch_power))}")
    z_scores = np.abs(psd_scaled - median_ch_power)/(std_power + 1e-12)#incase std_power is zero
    bad_indices = np.where(z_scores > 2.0)[0]
    bad_var_indices = np.where(var_ch_power == min(var_ch_power))[0]
    print("bad_indices: ", bad_indices)
    print("bad_var_indices: ", [ch_names[val] for val in np.argsort(var_ch_power)[::-1]])
    print([ch_names[idx] for idx in list(set(bad_indices))])
    print(ch_names[np.argmax(var_ch_power)])
    print(ch_names[np.argmin(var_ch_power)])
    idx = [i for i, name in enumerate(ch_names) if name == 'TP10']
    print(ch_names[29])
    print(var_ch_power[29])
    print(z_scores[29])
    length = len(freq) 
    counts = Counter(bad_indices)
    print(length)
    ...
    min_data = np.min(data, axis=1)
    max_data = np.max(data, axis=1)
    print(f"Min shape: {min_data.shape}\nMax shape: {max_data.shape}")
    bad_amp = np.where((max_data - min_data) < 5e-6)
    print([ch_names[b] for b in bad_amp[0]])
    print(np.min((max_data-min_data)))
    #Bad contact




    # group_mad = np.median(np.abs(mean_ch_power - group_median))
    # robust_sd = 1.4826 * group_mad

    # z_scores = np.abs(mean_ch_power - group_median) / robust_sd
    # bad_indices = np.where(z_scores > 3.0)[0]
    # print(bad_indices)
    # new_bads = [ch_names[idx] for idx in bad_indices]
    # print(new_bads)
    # Plot all channels across frequency
    plt.figure(figsize=(8, 4))
    # plt.plot(freq, psd_db.T, color='black', alpha=0.3)
    plt.plot(freq, mean_ch_power.T, color='red', alpha=0.5, label='mean_power')
    plt.plot(freq, median_ch_power.T, color='blue', alpha=0.5, label='median_power')
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("Power Spectral Density (dB)")
    plt.title("PSD Across All Channels")
    plt.grid(True)
    plt.show()

    #Implement the flatness logic for the autoreject_bads_ch
    #Look for other ways of verifying instead of p2p
    #Nibi cluster back at 5pm today