...
path = f"/Volumes/NINET/CARTBIND_EEG_data/UHN"
save_path = f"/Volumes/NINET/CARTBIND_EEG_data/CARTBIND_UHN"
...
def ica_remove_HVEOG(signal, report):
    ...
    ica = ICA(n_components=0.999, #For this location specifically, the default variance puts in infinite loop
                  random_state=97, 
                  max_iter="auto",
                  method='infomax', 
    ...
    eog_indices, eog_scores = ica.find_bads_eog(ica_train, ch_name=['EOG'])
...
def crop_signal(raw, samp_freq=2000):
    samp_freq = samp_freq
...
def autoreject_bads_ch(signal, report, zscore_thresh, samp_freq, n_fft=2048, fmax=45.0):
    ...
    bad_bounds_idx = np.where(bad_contact_prob >= 0.3)[0]
...
def main():
    ...
    folders = glob(f"{path}/*")
    epoch_num = []
    for folder in sorted(folders)[1:2]:
        ...
        for file in subfolders:
            if os.path.basename(file).split(".")[-1] != "cnt":
                continue

            base_file = os.path.basename(file)
            index = os.path.splitext(base_file)[0].split('_')[-1]
            time = time_dic[index]
            #MNE Report
            report_title = f"UHN_CARTBIND_{ID}_{time}"
            ...
            save_result(obj=cleaned_signal, save_path=f"{save_path}/{time}_filtered_eeg", 
                        title=f"CBN02_{ID}_REST_EC_{time}_UHN_filtered_eeg.fif", overwrite=True)
            ...
            save_result(obj=ar_epochs, save_path=f"{save_path}/{time}_epoch", 
                        title=f"CBN02_{ID}_REST_EC_{time}_UHN_epo.fif", overwrite=True)
            #Save mne Report
